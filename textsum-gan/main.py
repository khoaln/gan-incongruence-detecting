...
def build_seqgan_graph(hps, vocab):
    ...
    with tf.device('/gpu:0'):
        ...
        discriminator = Discriminator(sequence_length=hps.max_dec_steps.value,
                                      num_classes=2,
                                      vocab_size=FLAGS.vocab_size,
                                      embedding_size=hps.emb_dim.value,
...
def main(args):
    ...
    if hps.mode.value == "pretrain" or hps.mode.value == "train":
        generator, discriminator = build_seqgan_graph(hps, vocab)
        setup_training(hps.mode.value, generator, discriminator, generator_batcher, discriminator_batcher)
