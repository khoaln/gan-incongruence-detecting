...
class Generator(object):
    ...
    def _add_placeholders(self):
        ...
        if FLAGS.seqgan:
            self.D_reward = tf.placeholder(shape=[hps.batch_size.value, FLAGS.max_dec_steps], dtype=tf.float32)
    ...
    def _add_decoder(self, inputs):
        ...
        """
        hps = self._hps
        self.dec_cell = tf.contrib.rnn.LSTMCell(hps.hidden_dim.value, state_is_tuple=True,
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists, p_gens, multi_batch_num=1):
        ...
        with tf.variable_scope('final_distribution'):
            ...
            extra_zeros = tf.zeros((self._hps.batch_size.value*multi_batch_num, self._max_art_oovs))
            ...
            batch_nums = tf.range(0, limit=self._hps.batch_size.value*multi_batch_num)  # shape (batch_size)
            ...
            shape = [self._hps.batch_size.value*multi_batch_num, extended_vsize]
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('embedding'):
                self.embedding = tf.get_variable('embedding', [vsize, hps.emb_dim.value],
                                                 dtype=tf.float32, initializer=self.trunc_norm_init)
                if hps.mode.value == "train" or hps.mode.value == "pretrain":
            ...
            with tf.variable_scope('output_projection'):
                self.w = tf.get_variable('w', [hps.hidden_dim.value, vsize], dtype=tf.float32,
                                         initializer=self.trunc_norm_init)
            ...
            if hps.mode.value in ['pretrain', 'train', 'eval']:
                # Calculate the loss
                self.output_token = []
    ...
    def _add_train_op(self):
        ...
        # Apply optimizer
        optimizer = tf.train.AdamOptimizer(self._hps.lr.value)
    ...
    def build_graph(self):
        ...
        if self._hps.mode.value == 'train'or self._hps.mode.value == 'pretrain':
            self._add_train_op()
            self._rollout()
